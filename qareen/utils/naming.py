...
def get_collection_name(
    ...
    if not dataset_part:
        raise ValueError(f"dataset_name '{dataset_name}' yields empty sanitized value")
    if not model_part:
        raise ValueError(f"model_id '{model_id}' yields empty sanitized value")
    ...
    if len(dataset_part) + len(model_part) <= available_length:
        ...
        if available_for_parts <= 0:
            raise ValueError("Not enough length budget to encode dataset and model parts")
