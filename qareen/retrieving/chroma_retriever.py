...
class ChromaRetriever:
    ...
    def query_multimodal(
        ...
        if distance_metric is None:
            raise ValueError(
                "Collection metadata is missing the 'hnsw:space' key, so the distance metric "
                "cannot be verified. Please re-index the collection to ensure 'cosine' distance."
            )
        ...
        if metadatas_list and len(metadatas_list) != count:
            logger.warning("Mismatch in metadatas length: %d vs %d ids", len(metadatas_list), count)
        if docs_list and len(docs_list) != count:
            logger.warning("Mismatch in documents length: %d vs %d ids", len(docs_list), count)
        if distances_list and len(distances_list) != count:
            logger.warning("Mismatch in distances length: %d vs %d ids", len(distances_list), count)
