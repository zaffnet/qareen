...
class TestFusionRetrieval(unittest.TestCase):
    def setUp(self):
        ...
        # Mock get_collection_name to avoid validation logic during tests
        self.indexer.get_collection_name = MagicMock(return_value="mock_collection")
        self.indexer.create_vectorstore = MagicMock()
    ...
    def test_query_rrf_combines_scores(self):
        ...
        with patch.object(self.indexer, '_query_linear', side_effect=mock_query_linear):
            ...
            # doc2 (rank 2 in text, rank 1 in image): 0.5 * (1/62) + 0.5 * (1/61) = 0.0080645 + 0.0081967 = 0.0162612
    ...
    def test_l2_distance_metric_passed(self):
        # Verify distance_metric arg is passed down to create_vectorstore in RRF
         with patch.object(self.indexer, '_query_linear') as mock_linear:
            ...
            calls = self.indexer.create_vectorstore.call_args_list
