...
def _alpha_argument_type(value: str) -> float:
    try:
        parsed = float(value)
    except ValueError as exc:  # pragma: no cover - argparse handles messaging
        raise argparse.ArgumentTypeError("Alpha values must be numeric.") from exc
    if not 0.0 <= parsed <= 1.0:
        raise argparse.ArgumentTypeError("Alpha values must be between 0 and 1 inclusive.")
    return parsed
