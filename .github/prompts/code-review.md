...
- PR Head SHA: $GH_PR_HEAD_SHA
- PR Base SHA: $GH_PR_BASE_SHA
- Blocking Review: $GH_BLOCKING_REVIEW
...
Procedure:
- First, verify that you have the necessary capabilities:
  - Check if you can execute shell commands (gh CLI, jq)
  - Confirm access to the repository and PR context via environment variables
  - If any capability is missing or commands fail, abort immediately and output: "❌ Error: Missing required capabilities or tools (gh CLI, jq, or shell execution). Cannot perform automated code review. Please check agent environment or provide precomputed artifacts (PR JSON, diff patches, file patches)."
- Get existing comments: gh pr view --json comments
- Get diff: gh pr diff
- Get changed files with patches to compute inline positions: gh api repos/$GH_REPO/pulls/$GH_PR_NUMBER/files --paginate --jq '.[] | {filename,patch}'
...
- If CURRENT run finds issues and any prior "no issues" comments exist:
  - Prefer to remove them to avoid confusion:
    - Delete top-level issue comments via: gh api -X DELETE repos/$GH_REPO/issues/comments/<comment_id> (do not mask failures with || true; handle errors explicitly)
...
- If there ARE issues to report, submit ONE review containing ONLY inline comments plus an optional concise summary body. Use the GitHub Reviews API to ensure comments are inline:
  ...
  - Submit via: gh api repos/$GH_REPO/pulls/$GH_PR_NUMBER/reviews -f event=COMMENT -f body="$SUMMARY" -f comments='[$COMMENTS_JSON]'
- Do NOT use: gh pr review --approve or --request-changes

Blocking behavior:
- If BLOCKING_REVIEW is true and any 🚨 or 🔒 issues were posted, emit: CRITICAL_ISSUES_FOUND: true
- Otherwise emit: CRITICAL_ISSUES_FOUND: false
- Always emit the CRITICAL_ISSUES_FOUND marker at the very end of your output as the last line
- Format: exactly "CRITICAL_ISSUES_FOUND: true" or "CRITICAL_ISSUES_FOUND: false" on its own line
