...
if [[ "$STEP" == "all" ]] || [[ "$STEP" == "prepare" ]]; then
    ...
    if [ -d "$DATASET_PATH" ]; then
        ...
        python scripts/prepare_marqo_dataset.py || { echo "ERROR: Dataset preparation failed" >&2; exit 1; }
    fi
fi
...
if [[ "$STEP" == "all" ]] || [[ "$STEP" == "index" ]]; then
    ...
    for MODEL in "${MODELS[@]}"; do
        ...
        set_common_env
        MODELS_JSON=$(python -c "import json, sys; print(json.dumps([sys.argv[1]]))" "$MODEL") || { echo "ERROR: Failed to encode model" >&2; exit 1; }
        export QAREEN_EMBEDDING_MODELS="$MODELS_JSON"
        ALPHA_VALUES_STR=$(python -c "import json, sys; print(json.dumps([float(x) for x in sys.argv[1:]]))" "${ALPHA_VALUES[@]}") || { echo "ERROR: Failed to encode alpha values" >&2; exit 1; }
        export QAREEN_ALPHA_VALUES="$ALPHA_VALUES_STR"
        python scripts/build_index.py --dataset-name "$DATASET_PATH" || { echo "ERROR: Index building failed for $MODEL" >&2; exit 1; }
    done
fi

if [[ "$STEP" == "all" ]] || [[ "$STEP" == "visualize" ]]; then
    echo "Step 3: Generating Visualization"
    MODELS_JSON=$(python -c "import json, sys; print(json.dumps(sys.argv[1:]))" "${MODELS[@]}") || { echo "ERROR: Failed to encode models" >&2; exit 1; }
    ALPHAS_JSON=$(python -c "import json, sys; print(json.dumps([float(x) for x in sys.argv[1:]]))" "${ALPHA_VALUES[@]}") || { echo "ERROR: Failed to encode alpha values" >&2; exit 1; }
    set_common_env
